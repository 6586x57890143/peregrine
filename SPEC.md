...
internal/legacy/         legacy.go plus reactor.go, the step table. Shrinking; M13 deletes it
...
| 11c | The plugin move | **M10b's deferred half.** `internal/names`, `internal/learn` and `internal/generate` extracted first, because the reactor steps cannot move until what they share does; then each step to `internal/plugins/{chat,aggro,images,games,autopost}` with the state it closes over. `core.Deps` grows a `Guard`. Mechanical: no behaviour change, so every test that passes before passes after | |
