...
14. ~~Channel fan-out is unbounded, one goroutine per channel per guild, none visible to the shutdown path. The active-channel scan also pages every channel to count, then the ingest pass pages it again.~~ **Closed in M9.** Both levels are `errgroup.SetLimit`, from `PEREGRINE_INGEST_GUILD_CONCURRENCY` and `PEREGRINE_INGEST_CHANNEL_CONCURRENCY`, and the pass takes a context so shutdown stops it.
    ...
    `getActiveChannels` survives, because two callers want something a cursor cannot tell them: where people are *talking*, rather than what the bot has not read. The autonomous poster picks somewhere to speak and `findRandomActiveUser` picks a target. Its fan-out was the same unbounded goroutine-per-channel and is now bounded by the same variable, and its result is sorted, because completion order was nondeterministic and one caller picks by index: the bot's choice of channel depended on which REST call happened to return first. Turning its paging into a state-cache lookup belongs to M11, which owns both callers.
...
| 11b | The rest of engagement, and the plugin move | Image reposting given per-author caps and the deleted-message rule (A7); autonomous posting turned on deliberately; `getActiveChannels` paging becomes a state-cache lookup, which M9 left here; emote output verified against golden samples. **Carries M10b's deferred half:** each reactor step moves to `internal/plugins` together with the state it closes over. Closes 19, A7, the rest of §6 | |
...
- **Should a winning word-game guess be consumed?** It is not, today, so the guess still reaches the reply and learn steps as it always did. But the win path DELETES the guess message, and learning something the bot then removed from the channel is at least odd. Changing it is a behaviour change with no finding behind it, so M10b preserved the existing behaviour rather than smuggling one in; decide it with M11, which owns the word games.
