...
| 11 | Engagement features, and the plugin move | Word games un-darked, with the three orphan `go func` plus `Sleep` timers made ctx-bound so they stop firing against a closed session after shutdown; aggro, image reposting and autonomous posting given real config and turned on deliberately; per-author image caps and the deleted-message rule; emote output verified. **Carries M10b's deferred half:** each reactor step moves to `internal/plugins` together with the state it closes over, which is what makes this row the right place for it. Closes 19, A7, §6 | |
