...
// performDatabaseCleanup iterates through the Markov bucket and removes spammy keys and values.
func performDatabaseCleanup() {
	...
	if err != nil {
		log.Fatalf("[CLEANUP] Failed to open DB: %v", err)
	}
	...
	if err != nil {
		log.Fatalf("[CLEANUP] Database cleaning failed: %v", err)
	}
