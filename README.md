...
**Milestones 0 through 11b are complete**, except M8 which is dropped: repo hygiene and CI/CD, the `cmd/bot` entrypoint, `internal/config`, the `internal/core` lifecycle, `internal/text` plus `internal/filter`, `internal/safety`, the storage layer (`internal/corpus`, `internal/storage`, `internal/dbtest`, `internal/maintenance`), the generation engine `internal/markov`, the send chokepoint `internal/discordguard`, the message reactor, `internal/ingest`, `internal/wordgame` and `internal/activity`. M11c (the move to `internal/plugins`) through M13 are outstanding.
