...
### Clustering is being deleted, not rebuilt, and the reasoning matters

**M8 is dropped.** `SPEC.md` §8 finding 29 has the full case; the short version is four independent reasons, and the second is the one that turns "not worth it" into "actively wrong":
...
The one idea worth keeping is *transitive* association, which co-occurrence gives only the first hop of. That becomes a bounded query-time two-hop tier in M7b's seed scorer: never stale, no bucket, no codec, no persistence, and a dial golden samples can judge. `PEREGRINE_ENABLE_CLUSTERING` and `PEREGRINE_CLUSTERING_TICK` are deleted rather than returned to `Config`.
...
### How clustering got here (kept because the failure modes recur)
...
**Length is still legacy's, and is M7b's row.** The bound is `30 + rand(15)` words, which is a paragraph, governed by three competing mechanisms. Short and punchy reads as a joke; long reads as a malfunction. The engine only penalizes the end sentinel below the floor; the floor itself and the cap are enforced in `generateSentenceAttempt`. Do not tune the engine against these bounds, because they are about to move.
