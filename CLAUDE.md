...
**`Guard.EditEmbed` and `Guard.RespondModal` arrived with the wheel (M35) and needed no bypass-list entry**, because `ChannelMessageEditComplex` and `InteractionRespond` were already on it. `EditEmbed` is the repaint nobody pressed for (a turn timing out), which `UpdateEmbed` cannot do because it only answers a press; it is gated over every embed field like `SendEmbed`, and it ALWAYS sends the components pointer, because discordgo omits a nil one and a finished game would keep a row of buttons that can only fail. `RespondModal` goes through `permit` although its title is a repo constant: what that buys is the pause switch and the ignore list, since a form opened during a pause collects an answer whose result would be refused anyway.
